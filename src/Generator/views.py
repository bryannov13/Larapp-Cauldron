...
class index_generator():
    ...
    def create(self, path, default_fields=False):
        ...
        self.txt.append('\t\t\t\t\t\t\t<a href="/'+self.model_name+'/create" class="btn btn-primary-ws pull-right btn-wd">\n')
    ...
    def _set_fields(self,default_fields:bool=True)->list:
        ...
        for i,field in enumerate(self.fields):
            ...
            if field['type'] == 'Integer' or field['type'] == 'Floating':
                arra_txt.append('\t\t\t\t\t\t\t\t\t\t\t<input title="'+title+'" type="number" name="'+field['name']+'" id="'+field['name']+'" class="form-control" value="{{old("'+field['name']+'", isset($record) ? $record->'+field['name']+' : "")}}" data-parsley-required data-parsley-type="number" min="0" max="9999" maxlength="12">\n')
